...
MVCK.UI.UserPermissionPanel = ISPanel:derive("MVCK.UI.UserPermissionPanel")

function MVCK.UI.UserPermissionPanel:initialise()
	ISPanel.initialise(self)
end

function MVCK.UI.UserPermissionPanel:close()
	ISPanel.close(self)
end

function MVCK.UI.UserPermissionPanel:prerender()
    ISPanel.prerender(self)
end

function MVCK.UI.UserPermissionPanel:btnCancel_onClick(btn)
    self.close(self)
end

function MVCK.UI.UserPermissionPanel:btnConfirm_onClick(btn)
    if MVCK.dbByVehicleSQLID[self.vehicleID] then
        local temp = {}
        local count = 0
        for i = 1, #self.lblSet do
            ...
            if MVCK.dbByVehicleSQLID[self.vehicleID][self.chkBox[i].internal] then
                test = true
            else
                test = false
            end
        ...
        if count > 0 then
            ...
            sendClientCommand(getPlayer(), "MVCK", "updateSpecifyVehicleUserPermission", temp)
        end
    end
...
function MVCK.UI.UserPermissionPanel:createChildren()
    local btnWidth = 50 * MVCK.getUIFontScale()
    local bthHeight = 30 * MVCK.getUIFontScale()

    self.btnCancel = ISButton:new(self.width / 2 - btnWidth - 15, self.height - bthHeight - pad, btnWidth, bthHeight, "Cancel", self, MVCK.UI.UserPermissionPanel.btnCancel_onClick)
    ...
    self.btnConfirm = ISButton:new(self.width / 2 + 15, self.height - bthHeight - pad, btnWidth, bthHeight, "OK", self, MVCK.UI.UserPermissionPanel.btnConfirm_onClick)
    ...
    local lblwidth = getTextManager():MeasureStringX(UIFont.NewSmall, getText("IGUI_MVCK_User_Permissions_lblPublicPermissions"))
    self.lblPublicPermissions = ISLabel:new((self.width / 2) - (lblwidth / 2), pad, getTextManager():getFontHeight(UIFont.NewSmall), getText("IGUI_MVCK_User_Permissions_lblPublicPermissions"), 1, 1, 1, 1, UIFont.NewSmall, true)
    self.lblPublicPermissions:initialise()
    self.lblPublicPermissions:instantiate()
    self:addChild(self.lblPublicPermissions)

    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowDrive"), "AllowDrive")
    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowPassenger"), "AllowPassenger")
    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowSiphonFuel"), "AllowSiphonFuel")
    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowUninstallParts"), "AllowUninstallParts")
    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowAttachVehicle"), "AllowAttachVehicle")
    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowDetechVehicle"), "AllowDetechVehicle")
    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowTakeEngineParts"), "AllowTakeEngineParts")
    self:addSets(getText("IGUI_MVCK_User_Permissions_lblAllowOpeningTrunk"), "AllowOpeningTrunk")
