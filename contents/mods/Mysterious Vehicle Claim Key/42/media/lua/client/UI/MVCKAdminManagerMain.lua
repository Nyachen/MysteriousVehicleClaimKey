...
MVCK.UI.AdminManagerMain = ISCollapsableWindow:derive("MVCK.UI.AdminManagerMain")

function MVCK.UI.AdminManagerMain:listDataOnMouseDown(x, y)
...
function MVCK.UI.AdminManagerMain:listOnSelectionChange()
...
function MVCK.UI.AdminManagerMain:drawData(y, item, alt)
...
function MVCK.UI.AdminManagerMain:listDataDrawText(str, x, y, r, g, b, a, font)
...
function MVCK.UI.AdminManagerMain:filterStringCompare(mainText, compareText)
    return checkStringPattern(compareText) and string.match(string.lower(mainText), string.lower(compareText))
end

function MVCK.UI.AdminManagerMain:onFilterChange()
    self.parent.listData:clear()
    for k, v in ipairs(self.parent.varData) do
        local add = true
        if not MVCK.UI.AdminManagerMain:filterStringCompare(v.OwnerPlayerID, self.parent.textFilterUsername:getInternalText()) then
            add = false
        end
        if add then
            if not MVCK.UI.AdminManagerMain:filterStringCompare(v.carFullName, self.parent.textFilterVehicleName:getInternalText()) then
                add = false
            end
        end
        if add then
...
function MVCK.UI.AdminManagerMain:initList()
    ...
    for k, v in pairs(MVCK.dbByPlayerID) do
        for ak, av in pairs(v) do
            if ak ~= "LastKnownLogonTime" then
                local carFullName = MVCK.dbByVehicleSQLID[ak].CarModel
                ...
                if not carFullName  then
                    carFullName = MVCK.dbByVehicleSQLID[ak].CarModel
                end
                table.insert(tempTable, {OwnerPlayerID = k, carFullName = carFullName,  vehicleID = ak, ExpireOn = os.date("%d-%b-%y, %H:%M:%S", (MVCK.dbByPlayerID[k].LastKnownLogonTime + (SandboxVars.MVCK.ClaimTimeout * 60 * 60))), Location = MVCK.dbByVehicleSQLID[ak].LastLocationX .. "," .. MVCK.dbByVehicleSQLID[ak].LastLocationY, ClaimDateTime = os.date("%d-%b-%y, %H:%M:%S", MVCK.dbByVehicleSQLID[ak].ClaimDateTime)})
            end
        end
    end
...
function MVCK.UI.AdminManagerMain:initialise()
	ISCollapsableWindow.initialise(self)
end

function MVCK.UI.AdminManagerMain:createChildren()
    ISCollapsableWindow.createChildren(self)

    self.listData = ISScrollingListBox:new(30 * MVCK.getUIFontScale() + 10, 20 + padTop + getTextManager():getFontHeight(UIFont.NewSmall), self.width - 40 * MVCK.getUIFontScale() - 5 + 1 * MVCK.getUIFontScale(), 360 * MVCK.getUIFontScale());
    ...
    -- Total width 840
    local width = 0
    self.listData:addColumn(getText("IGUI_MVCK_Admin_Manager_listUsername"), width) -- 160 width, 126 width actual text
    width = 160 * MVCK.getUIFontScale()
    self.listData:addColumn(getText("IGUI_MVCK_Admin_Manager_listVehicleID"), width) -- 60 width, title is 55 width actual
    width = 270 * MVCK.getUIFontScale()
    self.listData:addColumn(getText("IGUI_MVCK_Admin_Manager_listCarName"), width) -- 300 width, 46 characters max, we will cut off if too long
    width = 570 * MVCK.getUIFontScale()
    self.listData:addColumn(getText("IGUI_MVCK_Admin_Manager_listClaimedDate"), width) -- 120 width, 103 width actual text
    width = 700 * MVCK.getUIFontScale()
    self.listData:addColumn(getText("IGUI_MVCK_Admin_Manager_listLocation"),  width) -- 80 width, 74 width actual
    width = 785 * MVCK.getUIFontScale()
    self.listData:addColumn(getText("IGUI_MVCK_Admin_Manager_listExpireon"), width) -- 105 width, 103 width actual text
    self:addChild(self.listData)
    self.listData:setFont(UIFont.NewSmall, 5)

    self.lblFilter = ISLabel:new(41 * MVCK.getUIFontScale(), self.listData:getBottom() + 10, getTextManager():getFontHeight(UIFont.NewLarge), getText("IGUI_MVCK_Admin_Manager_lblFilter"), 1, 1, 1, 1, UIFont.NewLarge, true)
    self.lblFilter:initialise()
    self.lblFilter:instantiate()
    self:addChild(self.lblFilter)

    -- Just to use it as visual header
    self.listFilterHeader = ISScrollingListBox:new(40 * MVCK.getUIFontScale(), self.lblFilter:getBottom() + 24 * MVCK.getUIFontScale(), 452 * MVCK.getUIFontScale(), 0)
    self.listFilterHeader:initialise()
    self.listFilterHeader:instantiate()
    self.listFilterHeader.drawBorder = true
    self.listFilterHeader:addColumn(getText("IGUI_MVCK_Admin_Manager_listUsername"), 0) -- 150 width, 126 width actual text
    self.listFilterHeader:addColumn(getText("IGUI_MVCK_Admin_Manager_listCarName"), 150 * MVCK.getUIFontScale()) -- 300 width, 46 characters max
    self:addChild(self.listFilterHeader)
    self.listFilterHeader:setFont(UIFont.NewSmall, 5)
    
    self.textFilterUsername = ISTextEntryBox:new("", 40 * MVCK.getUIFontScale(), self.listFilterHeader:getBottom(), 151 * MVCK.getUIFontScale(), getTextManager():getFontHeight(UIFont.NewMedium))
    ...
    self.textFilterVehicleName = ISTextEntryBox:new("", 190 * MVCK.getUIFontScale(), self.listFilterHeader:getBottom(), 302 * MVCK.getUIFontScale(), getTextManager():getFontHeight(UIFont.NewMedium))
    ...
    local tempImage
    local tabBtnSize = 30 * MVCK.getUIFontScale()
    ...
    self.btnViewSafehouse:setEnable(false)
    self.btnViewSafehouse:setTooltip(getText("IGUI_MVCK_Admin_Manager_btnViewSafehouse_Tooltip"))
    self:addChild(self.btnViewSafehouse)

    tempImage = getTexture("media/ui/MVCK_factions.png")
    self.btnViewFaction = ISButton:new(6, tabBtnSize * 1 + 10 + getTextManager():getFontHeight(UIFont.NewSmall) + 2 + 1 * MVCK.getUIFontScale(), tabBtnSize, tabBtnSize, "", self, self.btnOnClick)
    ...
    self.btnViewFaction:setEnable(false)
    self.btnViewFaction:setTooltip(getText("IGUI_MVCK_Admin_Manager_btnViewFaction_Tooltip"))
    self:addChild(self.btnViewFaction)

    tempImage = getTexture("media/ui/MVCK_modify.png")
    self.btnModifyPermissions = ISButton:new(6, tabBtnSize * 2 + 10 + getTextManager():getFontHeight(UIFont.NewSmall) + 4 + 1 * MVCK.getUIFontScale(), tabBtnSize, tabBtnSize, "", self, self.btnOnClick)
    ...
    self.btnModifyPermissions:setEnable(false)
    self.btnModifyPermissions:setTooltip(getText("IGUI_MVCK_Admin_Manager_btnModifyPermissions_Tooltip"))
    self:addChild(self.btnModifyPermissions)

    tempImage = getTexture("media/ui/MVCK_delete.png")
    self.btnDelete = ISButton:new(6, tabBtnSize * 3 + 10 + getTextManager():getFontHeight(UIFont.NewSmall) + 6 + 1 * MVCK.getUIFontScale(), tabBtnSize, tabBtnSize, "", self, self.btnOnClick)
    ...
    self.btnDelete:setEnable(false)
    self.btnDelete:setTooltip(getText("IGUI_MVCK_Admin_Manager_btnDelete_Tooltip"))
...
function MVCK.UI.AdminManagerMain:btnOnClick(btn)
    ...
    elseif btn.internal == "btnModifyPermissions" then
        self.panelModify = MVCK.UI.UserPermissionPanel:new((getCore():getScreenWidth() / 2) - (200 * MVCK.getUIFontScale() / 2), (getCore():getScreenHeight() / 2) - (300 * MVCK.getUIFontScale() / 2), 200 * MVCK.getUIFontScale(), 300 * MVCK.getUIFontScale(), self.listData.items[self.listData.selected].item.vehicleID)
    ...
    elseif btn.internal == "btnDelete" then
        ...
        self.modDialog = ISModalDialog:new((getCore():getScreenWidth() / 2) - (250 / 2), (getCore():getScreenHeight() / 2) - (100 / 2), 250, 100, message, true, self, MVCK.UI.AdminManagerMain.btnUnclaim_onConfirmClick, getPlayer():getPlayerNum(), nil)
        self.modDialog:initialise()
        self.modDialog:addToUIManager()
    end
end

function MVCK.UI.AdminManagerMain:btnUnclaim_onConfirmClick(btn, _, _)
    if btn.internal == "NO" then return end
    sendClientCommand(getPlayer(), "MVCK", "unclaimVehicle", { self.listData.items[self.listData.selected].item.vehicleID })
...
function MVCK.UI.AdminManagerMain:removeUsernameVehicleID(OwnerPlayerID, vehicleID)
...
function MVCK.UI.AdminManagerMain:close()
    ...
    if MVCK.UI.AdminInstance then MVCK.UI.AdminInstance = nil end
    self:removeFromUIManager()
end

function MVCK.UI.AdminManagerMain:prerender()
	ISCollapsableWindow.prerender(self)
end

function MVCK.UI.AdminManagerMain:render()
	ISCollapsableWindow.render(self)
end

function MVCK.UI.AdminManagerMain:new(x, y, width, height)
    ...
    o.title = getText("IGUI_MVCK_Admin_Manager_Title")
    o.width = width
    o.height = height
	o.visibleTarget	= o
